...
export const createNewPost = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        const { description, type_privacy }: INewPost = req.body
        ...
        await conn.query(
          'INSERT INTO posts (uid, type_privacy,description ,person_uid) value (?,?,?,?)',
          [uidPost, type_privacy, description, req.idPerson]
        )
...
export const unSavePostByUser = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    const { post_save_ui }: IUnSavePost = req.body

    const conn = await connect()

    await conn.query(
      'DELETE FROM post_save WHERE post_save_uid = ?',
      [post_save_ui]
    )
